...
namespace BookVault.Shared
{
    ...
    {
        public static string GenerateThumbnail(string pdfPath, string thumbnailsDir, int page = 0, int width = 200, int height = 200)
        {
            using var stream = File.OpenRead(pdfPath);
            using var pdfDocument = PdfDocument.Load(stream);
            using var image = pdfDocument.Render(page, width, height, true);

            string thumbnailFilename = Path.GetFileNameWithoutExtension(pdfPath) + $"-thumbnail.png";
