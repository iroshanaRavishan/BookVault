    ...
    public class FilmDbContext(DbContextOptions<FilmDbContext> options) : DbContext(options)
    {
        ...
        protected override void OnConfiguring(DbContextOptionsBuilder optionsBuilder)
        {
            optionsBuilder
                ...
                {
                    var sampleFilm = await context.Set<Film>().FirstOrDefaultAsync(b => b.Name == "Harry Potter 12-i");
                    if (sampleFilm == null)
                    {
                        sampleFilm = Film.Create("Harry Potter 12-i", "Fantasy", new DateTimeOffset(new DateTime(2025, 1, 3), TimeSpan.Zero), 7);
                ...
                {
                    var sampleFilm = context.Set<Film>().FirstOrDefault(b => b.Name == "Harry Potter 12-i");
                    if (sampleFilm == null)
                    {
                        sampleFilm = Film.Create("Harry Potter 12-i", "Fantasy", new DateTimeOffset(new DateTime(2025, 1, 3), TimeSpan.Zero), 7);
